...
public class Util {
    ...
    public static boolean getCurrentTask(Context context){
        ...
        List<ActivityManager.RunningTaskInfo> appProgressInfo = activityManager.getRunningTasks(Integer.MAX_VALUE);
        for (ActivityManager.RunningTaskInfo progress : appProgressInfo){
            if (progress.baseActivity.getPackageName().equals(context.getPackageName()) ||
                    progress.topActivity.getPackageName().equals(context.getPackageName())){
                return true;
            }
        }
        return false;
